...
void GrowtopiaBot::onLoginRequested()
{
	...
	string packet = "tankIDName|" + uname + "\ntankIDPass|" + upass + "\nrequestedName|SmellBurp\nf|1\nprotocol|146\ngame_version|3.73\nfz|11411280\nlmode|0\ncbits|0\nplayer_age|58\nGDPR|1\ncategory|_-5100\nhash2|" + hash2 + "\nmeta|" + generateMeta() + "\nfhash|-716928004\nrid|" + generateRid() + "\nplatformID|0\ndeviceVersion|0\ncountry|us\nhash|" + hash + "\nmac|" + generateMac() + "\nwk|328A5A9FFE0FE9C90FE00CFA15BC6484" + token;
	cout << packet;
	SendPacket(2, "tankIDName|" + uname + "\ntankIDPass|" + upass + "\nrequestedName|SmellBurp\nf|1\nprotocol|146\ngame_version|3.73\nfz|11411280\nlmode|0\ncbits|0\nplayer_age|58\nGDPR|1\ncategory|_-5100\nhash2|" + hash2 + "\nmeta|" + generateMeta() + "\nfhash|-716928004\nrid|" + generateRid() + "\nplatformID|0\ndeviceVersion|0\ncountry|us\nhash|" + hash + "\nmac|" + generateMac() + "\nwk|328A5A9FFE0FE9C90FE00CFA15BC6484" + token, peer);
...
void GrowtopiaBot::userLoop() {
	...
	if (peer) SendPacket(3, "action|world_button\nname|_0", peer);
	timeFromWorldEnter++;
	counter++;
	if ((counter % 1800) == 0)
	{
